...
export class InputStyleDirective {
    ...
    onFocus() {
      if (!this.elementRef.nativeElement.disabled) {
        this.elementRef.nativeElement.style.borderColor = this.styleSettings?.globalInputBorderColor;
        this.elementRef.nativeElement.style.boxShadow = `0 0 0 0.25rem ${this.styleSettings?.globalInputBorderColor.slice(0, 7)}40`;
      }
    }
    ...
    constructor(private styleService: StyleService, private elementRef: ElementRef) { 
        this.styleService.styleSettings$.pipe(takeUntilDestroyed()).subscribe(ss => {
            ...
            this.elementRef.nativeElement.style.borderColor = ss.globalInputBorderColor;
